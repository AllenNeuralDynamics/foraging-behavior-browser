...
def _draw_variable_trial_back(df, beta_name, trials_back, ax=None):
    ...
    axes[1].set_title(f"{len(df_beta['subject_id'].unique())} mice, {len(df_beta['session'].unique())} sessions")
...
def _draw_variable_trial_back_linear_reg(df, beta_name, ax=None):
    ...
    ax.set_title(f"{beta_name}, {len(df_beta['subject_id'].unique())} mice, {len(df_beta['session'].unique())} sessions")
